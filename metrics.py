...
#def ade(predAll,targetAll,count_):
#    sum_list=[]
#    pred = np.swapaxes(predAll[0][:,:count_[0],:],0,1)  
#    target = np.swapaxes(targetAll[0][:,:count_[0],:],0,1)
#    N = pred.shape[0]
#    T = pred.shape[1]
#    sum_ = 0 
#    for t in range(T):
...
#    for i in range(N):
#        t=T-1
#        sum_1=math.sqrt((pred[i,t,0] - target[i,t,0])**2+(pred[i,t,1] - target[i,t,1])**2)
#    sum_all = sum_1/(N)
#    return sum_all,sum_list
